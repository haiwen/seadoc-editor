...
class SelectOptionGroup extends Component {
  ...
  renderOptGroup = (searchVal) => {
    let { noOptionsPlaceholder, onSelectOption, selectedOption, hasIcon } = this.props;
    ...
    return this.filterOptions.map((option, index) => {
      ...
      return (
        ...
        >
          ...
          {isSelected && <i className="sdocfont sdoc-check-mark text-gray font-weight-bold"></i>}
        </Option>
      );
    });
  };
  ...
  render() {
    const { top, left, minWidth, isInModal, position, className } = this.props;
    ...
    return (
      <div
        className={classnames('sdoc-options option-group', className ? 'option-group-' + className : '')}
